 ...
 *
 * 对齐 st-api-wrapper/src/apis/preset/types.ts
 */
...
/**
 * 旧酒馆（SillyTavern 原始结构）包装构建参数：
 * 先转换为新格式，再执行 buildPrompt。
 */
export interface BuildPromptFromSillyTavernParams {
  /** ST 原始 preset（settings）对象 */
  preset: any;
  ...
  options?: BuildPromptParams['options'];
}
