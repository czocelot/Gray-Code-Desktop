...
export function useCheckpointConfig() {
  ...
  function configFieldEquals(a: unknown, b: unknown): boolean {
    ...
    if (Array.isArray(a) || Array.isArray(b)) {
      return Array.isArray(a)
        && Array.isArray(b)
        && a.length === b.length
        && a.every((value, index) => configFieldEquals(value, b[index]))
    }
    ...
    return leftKeys.length === rightKeys.length
      && leftKeys.every((key, index) => key === rightKeys[index] && configFieldEquals(left[key], right[key]))
  }
