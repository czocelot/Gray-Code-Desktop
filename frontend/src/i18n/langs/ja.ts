...
const ja: LanguageMessages = {
    ...
    components: {
        ...
        update: {
            title: '新バージョンがあります',
            intro: 'Gray Code v{version} が公開されました。「設定 → 一般 → 自動更新」でバージョンを選んで手動で更新してください。',
            releaseNotes: '更新内容',
        ...
        settings: {
            ...
            settingsPanel: {
                ...
                update: {
                    title: '自動更新',
                    description: '起動時に GitHub Releases の新バージョンを確認します（24時間以内の再確認は行いません）。新しいバージョンは更新パッケージをダウンロードして手動で更新できます。',
                    enableLabel: '自動更新チェックを有効にする',
                    kindLabel: 'ダウンロードするバージョン',
                    kindAuto: '自動（現在の実行形態に従う）',
                    kindPortable: 'ポータブル版（GrayCode-Portable）',
                    kindInstalled: 'インストーラー版（GrayCode.Setup）',
                    kindHint: 'ダウンロードする更新パッケージの形態を選択します。「自動」は現在の実行形態に従います（ポータブルはポータブル版、インストーラー版はインストーラー版）。',
                    kindChanged: 'ダウンロードするバージョンを変更しました。「今すぐ確認」で新しい選択に合わせて再チェックしてください。',
                    checkNow: '今すぐ確認',
                    updateNow: '更新パッケージをダウンロード',
                    checking: '確認中...',
                    upToDate: '最新バージョンです',
                    updateAvailable: '新バージョン v{version} があります',
                    installedHint: 'v{version} の更新パッケージをダウンロードしました: {path}。残りの更新手順は手動で行ってください（ポータブル版は GrayCode-Portable、インストーラー版は GrayCode.Setup を実行）。',
                    error: '確認に失敗しました',
                ...
                memory: {
                    ...
                    rawEntries: {
                        ...
                        addPlaceholder: '記憶したい内容を入力し「記憶を追加」をクリックすると、手動で長期記憶に書き込まれます（AI の memory_note と同等）。Ctrl+Enter / ⌘+Enter でも送信できます。',
                        add: '記憶を追加',
                        added: '記憶 #{id} を追加しました',
