...
class TpsMeter {
  ...
  private sample(): void {
    ...
    const live = now - this.lastRecordAt <= LIVE_WINDOW_MS
    // 归零稳态去重：ema=0 且 !live 时 ring 全零滚动、内容不变——跳过通知，
    // 避免空闲期每 200ms 唤醒订阅者（TpsBar 空画布重绘循环）
    if (this.ema === 0 && !live && this.lastSample.ema === 0 && !this.lastSample.live) {
      const last = this.lastSample.ring
      if (this.ring.length === last.length && this.ring.every((v, i) => v === last[i])) {
        return
      }
    }
    this.lastSample = {
