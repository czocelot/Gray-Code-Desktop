...
### Fixed
  - **思考强度下拉框 1.7.8 初版修正**：初版仅提供 Off/Low/Medium/High 四档（档位被归并，设置页 none/minimal/xhigh/max/ultra/custom 等档位无法在快捷下拉中精确保留，选择一次即被覆盖），且 Off 与 openai `none` 语义混淆。已修正：按渠道提供与设置页一致的完整档位列表（精确读写、不归并）；Off/none 语义严格区分（**none = 完全不传递思考参数**：请求缺省该段，模型按 API 默认行为思考；**off = 强制传递禁用参数**——anthropic 显式携带 `{"thinking":{"type":"disabled"}}`、gemini 显式携带 `{"thinkingConfig":{"includeThoughts":false}}`（Gemini 缺省即思考，必须显式 false 才真正关闭）、openai 系对「闸门关闭 + effort='none'」强制透传 `{"reasoning":{"effort":"none"}}`（OpenAI 缺省 reasoning 段时模型仍按默认强度思考，只有显式 none 才真正关闭））；设置页思考模式/思考强度描述同步更新（en/zh-CN/ja 三语言）。
