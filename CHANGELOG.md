...
### Added：代码查看面板「打开为新页面」——同级标签页编辑文件（桌面版）
  ...
  - **编辑器背景半透明**：文件编辑器（含编辑区）背景接入全局半透明表面色（--gc-surface-editor-bg，随「UI 不透明度」设置 color-mix 混合透明），文字/图标保持全不透明，可透出桌面背景图/窗口背景。
...
### Fixed：[1.7.10dev] 远程控制 V7.1——模型管理列表刷新修复 + 图标渲染健壮性加固
  ...
  - **图标渲染健壮性加固**：全部 HTML 内联 SVG（16 处）与 JS 动态渲染图标（icon()/ICON_SEND/ICON_STOP）补显式 width/height 属性——即使外层 CSS 规则因任何原因未生效（样式表解析失败/变量缺失/旧浏览器），图标仍以合理尺寸显示，不再出现「退出按钮/加号/工作区加号空白」；
  - 测试：新增模型管理对话框全流程（打开强制重拉断言/添加模型刷新/删除模型刷新/请求体形状）+ 内联 SVG 尺寸属性断言，fixture 模拟 models-add/remove/model 端点写回。
...
### Added（1.7.10dev 补记：多会话并行 + 设置分页 + 流式/落盘修复）
  ...
  - **输入区渠道/模型选择**：输入框上方新增当前渠道+模型胶囊（桌面端 ChannelSelector 同款），点击弹出渠道与模型选择弹层（`/api/configs|config|model|channel-active`），与设置页「渠道」分类同一数据源。
  - **历史消息分页**：`GET /api/messages` 支持 `limit/offset`（默认 120，上限 500，自尾端向前开窗），移动端滚动到顶自动回溯加载更早历史，避免数百条消息一次性下发渲染；`GET /api/conversations` 支持 `limit/offset`（默认 30，上限 100），返回 `total/hasMore`，会话列表先按 `updatedAt` 降序全量排序再切片（分页跨页不乱序）。
  - **`POST /api/send` 返回 `streamId`**：新建会话页签与 SSE 流按 streamId 精确对齐，消除「POST 响应与首个 SSE chunk 先后不确定」的竞态窗口（配合移动端孤儿 chunk 缓冲补发）。

### Fixed（1.7.10dev 补记）
  - **远程控制「巨大『正在加载历史消息』挡住对话」**：`#messages`/`#empty`/`#file-viewer` 的 CSS 规则设置了 `display:flex`，覆盖了 UA 样式表的 `[hidden]{display:none}`——切换会话/加载历史时「加载中」巨块与消息同屏显示、挡住大半对话。新增全局 `[hidden]{display:none !important}` 兜底修复。
  - **远程控制流式输出断续/丢失（批量 chunk 被丢弃）**：桌面端 50ms 节流合并的 `streamChunkBatch`（`data` 为 chunk 数组、conversationId/streamId 位于每个元素上）此前被移动端当单对象处理全部丢弃，只剩偶发单 chunk 能渲染；现已按真实装配形状解析（兼容单/批量/包装层三种位置），批量流完整累积。
  - **远程控制会话摘要不落盘（列表 messageCount 恒 0 / 预览为空）**：桌面端的 `custom.messageCount/custom.preview` 由前端在流式完成后调用 `conversation.updateSummary` 写入，移动端没有该前端逻辑——远端创建的会话列表计数/预览永远缺失。服务器在流终结（complete/cancelled/error）后自动按实际历史补齐摘要（messageCount=历史总数、preview=最后一条非工具响应用户消息前 50 字符，与桌面端口径一致）。
  - **消息删除定位错误（历史分页后删除错消息）**：删除消息现使用服务端下发的绝对 `index`（`deleteSingleMessage` 按全量历史索引定位），不再使用窗口内可见下标。
  - **连接断开后页签卡死「生成中」**：SSE 断开（bye/error）时复位全部页签的流式状态与待批准工具，发送键不再永久禁用。
  - **设置分类标题缺失翻译**：`secRemote` 此前不在语言包中，远程控制设置节标题显示原始键名；已补齐三语言。
...
### Added
  - **远程控制（桌面版）**：设置页新增「远程控制」页签（`remoteControl` 配置，按机器保存，默认关闭）：
    - 启用后在局域网内监听自定义端口（1-65535，默认 17532），提供**移动端友好的自包含控制页**（零外部依赖，随服务器懒加载——关闭时服务器与 UI 完全不存在，不占任何资源）；
    - 手机上可查看/切换会话、发送消息、停止生成、重命名会话，实时流式输出（SSE）；桌面端自身的会话流式输出也会镜像到手机；
    - 移动端以 `remote-control` 客户端身份接入现有 MessageRouter，与桌面端同一消息管道（chatStream/cancelStream/会话管理等全部复用既有校验与工具循环）；
    - 安全基线：默认关闭（opt-in）、仅局域网可用；Host/Origin 校验（防 DNS rebinding 与跨站写滥用）、POST 仅接受 application/json、消息/请求体长度限制、会话读取只允许已存在会话（防穷举建目录磁盘 DoS）、SSE 并发上限 8 + 空闲超时；设置页展示本机全部局域网地址与安全说明；
...
### Fixed
  - **思考强度下拉框 1.7.8 初版修正**：初版仅提供 Off/Low/Medium/High 四档（档位被归并，设置页 none/minimal/xhigh/max/ultra/custom 等档位无法在快捷下拉中精确保留，选择一次即被覆盖），且 Off 与 openai `none` 语义混淆。已修正：按渠道提供与设置页一致的完整档位列表（精确读写、不归并）；Off/none 语义严格区分（**none = 完全不传递思考参数**：请求缺省该段，模型按 API 默认行为思考；**off = 强制传递禁用参数 `{"thinking":{"type":"disabled"}}`**——anthropic 显式写 `thinking.type='disabled'`、gemini 显式写 `thinkingConfig.includeThoughts=false`（Gemini 缺省即思考，必须显式 false 才真正关闭）、openai 系关闭闸门后请求强制携带 `{"thinking":{"type":"disabled"}}` 且**不传递任何 effort**（缺省 reasoning/thinking 段时模型仍按默认行为思考，只有显式禁用才真正关闭））；设置页思考模式/思考强度描述同步更新（en/zh-CN/ja 三语言）。
