...
### Changed
  - 瀛樻。鐐瑰垱寤?鎭㈠涓绘祦绋嬪垏鎹㈠埌鏂版灦鏋勶細鍒涘缓鏀圭敤 `CheckpointSnapshotBuilder`锛堝鏍瑰伐浣滃尯鎵弿銆佸己鍒舵帓闄ゅ瓨妗ｇ洰褰曡嚜韬€佹祦寮忓搱甯?+ 鏈夌晫骞跺彂銆乻tat 澶嶇敤锛夛紝鎭㈠鏀圭敤 `CheckpointRestoreEngine`锛堝閲忛摼鏂囦欢绱㈠紩 O(1) 鏌ヨ銆乻coped 璺緞瀹夊叏瑙ｆ瀽銆佸け璐ユ竻鍗曞尯鍒?missing_in_chain / hash_mismatch / copy_failed / delete_failed锛夛紱鏂板瓨妗ｈ褰曞伐浣滃尯韬唤锛坄workspaceRoots` / `workspaceFingerprint`锛夛紝鎭㈠鍓嶆牎楠屽綋鍓嶅伐浣滃尯锛岃法椤圭洰鎭㈠琚槑纭嫆缁濓紱鏂板瓨妗ｅ浠界洰褰曟敼鐢?scoped 甯冨眬锛坄cp_xxx/ws_xxx/relative`锛夛紝澶氭牴宸ヤ綔鍖哄悓鍚嶆枃浠朵笉鍐嶄簰鐩歌鐩栵紱鏃ф牸寮忓瓨妗ｏ紙鐩稿璺緞閿?+ 鏃у竷灞€锛夊崟鏍规仮澶嶄繚鎸佸吋瀹癸紝澶氭牴涓嬫槑纭嫆缁濊€岄潪闈欓粯閿欐仮澶嶏紱瀛樻。鍒涘缓/鎭㈠杩涘叆宸ヤ綔鍖虹骇浜掓枼閿侊紙绛夊緟杩涜涓殑鍐欏伐鍏烽€€鍑哄苟闃绘鏂板啓鍏ワ級锛屾仮澶嶅け璐ヨ矾寰勮浆鐩稿璺緞灞曠ず
  - 鐢ㄩ噺缁熻鎬ц兘鍐嶄紭鍖栵細缁熻璇诲彇鍏冩暟鎹敼涓鸿交閲忚矾寰勶紙鍙 `{id}.meta.json`锛屼笉鍐嶈蛋 `getMetadata` 鐨勫巻鍙插畬鏁存€ф鏌モ€斺€旀鍓嶆瘡娆＄粺璁￠兘浼氫负姣忎釜瀵硅瘽棰濆鍔犺浇涓€娆″巻鍙诧紝绱㈠紩浼樺寲鐨勬敹鐩婅鎶垫秷澶у崐锛夛紱鏂板瀵硅瘽鐩綍鐩戝惉锛坄fs.watch` recursive锛? 鍐呭瓨鏄庣粏缂撳瓨鈥斺€斾换浣曞巻鍙?鍏冩暟鎹?绱㈠紩鏂囦欢鍐欏叆閮戒細鎶婂搴斿璇濇爣璁颁负 dirty锛岀粺璁″彧閲嶈 dirty 瀵硅瘽骞跺洖濉紦瀛橈紝鍏朵綑瀵硅瘽鐩存帴閲嶆斁鍐呭瓨鏄庣粏锛堥浂 stat銆侀浂璇绘枃浠讹級锛屾棩甯哥粺璁′粠鍑犲崈娆¤法杩涚▼鏂囦欢璋冪敤闄嶅埌姣绾э紱缁熻鑷韩閲嶅缓绱㈠紩鍐欏叆 `{id}.usage.json` 浼氳Е鍙戜竴娆¤嚜浼ゆ爣璁帮紝涓嬩竴杞噸璇诲皬绱㈠紩鏂囦欢鍚庤嚜鐒舵仮澶嶏紝涓嶄細鏃犻檺寰幆锛涙墿灞?dispose 鏃堕噴鏀剧洃鍚紝闈炴枃浠跺瓨鍌紙娴嬭瘯/鍐呭瓨閫傞厤鍣級鑷姩閫€鍖栧叏閲忔壂鎻忥紱鏂板缂撳瓨鍛戒腑璺宠繃璇诲彇 / dirty 閲嶈鍥炲～ / 宸插垹瀵硅瘽娓呯悊 / 缂撳瓨鏃堕棿绛涢€変笌 watcher 鏂囦欢鍚嶈В鏋愭祴璇?  - 鎻愮ず璇嶈缃〉鎺掔増閲嶆帓锛圥R #5锛夛細鍔ㄦ€佷笂涓嬫枃淇濈暀绛栫暐鍖哄潡浠庡叏灞€鍥哄畾浣嶇疆绉诲叆瀵瑰簲鐨勬ā鏉挎ā寮忓唴鈥斺€旈璁炬潯鐩ā寮忎笅鏄剧ず鍦ㄦ潯鐩紪杈戝尯涓嬫柟锛屼紶缁熸ā鏉挎ā寮忎笅鍐呰仈鏄剧ず鍦ㄥ姩鎬佹ā鏉挎枃鏈涓嬫柟锛岄€夐」褰掑睘涓嶅啀璁╀汉鍥版儜锛涙柊澧炪€屽彲鐢ㄥ彉閲忓弬鑰冦€嶅彲鏀剁缉闈㈡澘锛堥粯璁ゆ敹璧凤紝闈欐€佸彉閲忕粍 / 鍔ㄦ€佸彉閲忕粍鍒嗙粍灞曠ず锛宑hevron 灞曞紑鏀惰捣锛夛紝闀垮彉閲忓垪琛ㄤ笉鍐嶆案涔呭崰鎹缃〉绌洪棿
  - 鎻愮ず璇嶆ā寮忔爮鏂板淇濆瓨鎸夐挳锛堢豢鑹蹭繚瀛樺浘鏍囷紝淇濆瓨涓垏鎹负 loading 鍔ㄧ敾锛夛紝涓庡簳閮ㄥ師淇濆瓨鎸夐挳骞跺瓨锛涘鍏?/ 瀵煎嚭鎸夐挳浠?codicon 閫氱敤鍥炬爣鏀逛负鎴愬鐨勮嚜瀹氫箟 SVG 鍥炬爣锛堟柟鍚戠浉鍙嶇殑瀵煎嚭/瀵煎叆绠ご锛夛紝瑙嗚涓婃槑纭负涓€瀵规搷浣?  - 鎻愮ず璇嶉〉淇濆瓨 / 瀵煎嚭 / 瀵煎叆鍙嶉缁熶竴涓烘诞绐?toast锛堟垚鍔?/ 澶辫触鐫€鑹诧紝2.5 绉掕嚜鍔ㄦ秷澶憋紝Transition 鍔ㄧ敾锛夛紝绉婚櫎搴曢儴琛屽唴鏂囨湰鎻愮ず锛坰aveMessage锛?  - 瀛樻。鐐规枃浠跺搱甯屾敼涓烘祦寮忚鍙栵紙`CheckpointManager.getFileHash` / `computeFileHashes`锛夛紝涓嶅啀鎶婂ぇ鏂囦欢鏁翠綋 `readFile` 杩涘唴瀛橈紝鍝堝笇缁撴灉涓嶅彉
  - 榛樿鍚敤瀛樻。鐨勫啓宸ュ叿鍒楄〃琛ラ綈锛圕P-13锛夛細鏂板 `insert_code`銆乣delete_code`銆乣search_in_files`锛坮eplace 妯″紡锛夈€佸浘鍍忓鐞嗭紙`remove_background` / `crop_image` / `resize_image` / `rotate_image`锛変笌鏂囨。绫伙紙`create_plan` / `update_plan` / `create_design` / `update_design` / `create_progress` / `update_progress` / `record_progress_milestone` / `create_review` / `record_review_milestone` / `finalize_review` / `reopen_review`锛夐粯璁ゅ湪鎵ц鍓?鍚庡垱寤哄瓨妗ｏ紱`search_in_files` 绾?search 妯″紡锛堥潪 replace锛変笉鍐嶅垱寤哄叏宸ヤ綔鍖哄瓨妗?  - 鎭㈠妫€鏌ョ偣鍓嶄富鍔ㄥ彇娑堣瀵硅瘽鐨勬祦寮忚姹備笌鍏宠仈鐨勬椿璺?SubAgent锛圕P-04/CP-12锛夛紝闃叉鎭㈠鍚庤繜鍒?chunk 姹℃煋鍘嗗彶銆丼ubAgent 缁х画鍐欐枃浠朵笌鎭㈠缁撴灉鍐茬獊
  - 瀛樻。鎺掗櫎鍔熻兘涓婄嚎锛圗X-01~EX-12锛夛細`CheckpointIgnoreResolver` 鍗囩骇涓哄洓灞傛帓闄ゆā鍨嬧€斺€斿己鍒舵帓闄わ紙`.git` / `node_modules` / 鎵╁睍瀛樺偍鏍圭粷瀵硅矾寰勶紝`!` 涓嶅彲鍚﹀畾锛夆啋 榛樿鎺掗櫎绫诲埆锛? 绫伙紝璁剧疆椤靛彲鍒嗗埆寮€鍏筹紝鍙 `!` 閲嶆柊绾冲叆锛夆啋 宓屽 `.gitignore`锛坅nchored + 鍚﹀畾 + 鐩綍浣滅敤鍩燂級鈫?鐢ㄦ埛鑷畾涔夋ā寮忥紙鐙珛鏈€缁堟眰鍊奸樁娈碉紝鏈€鍚庣敓鏁堬級锛沗shouldIgnore` 杩斿洖 `{ignored, reason, rule, source}`锛屽揩鐓ф瀯寤鸿緭鍑哄畬鏁?`excluded` 娓呭崟锛堝惈鍛戒腑瑙勫垯鏂囨湰锛変笌鎺掗櫎缁熻锛涘崟鏂囦欢澶у皬涓婇檺榛樿 50 MiB锛? = 涓嶉檺鍒讹級锛岃秴闄愭枃浠惰褰?`reason: 'size'` 涓嶉潤榛樻秷澶憋紱瀛樻。璁板綍涓?manifest 淇濆瓨鎺掗櫎瑙勫垯蹇収 `ignoreSnapshot` 涓?`excludedCount/excludedBytes`锛屾仮澶嶆椂瀵规瘮蹇収瑙勫垯涓庡綋鍓嶈鍒欒緭鍑?`excludedNote`锛堝惈 `rulesChanged`锛夛紝涓旀仮澶嶈繃婊や弗鏍兼寜褰撳墠瑙勫垯锛堜笉鍥犳棫瑙勫垯瀹借€岃鐩栧綋鍓嶆槑纭拷鐣ョ殑鏂囦欢锛?  - 瀛樻。鎺掗櫎璁剧疆涓庨瑙堬紙EX-08/EX-09锛夛細`CheckpointConfig.exclusion`锛坋nabledProfiles 鍏ㄥ紑 / maxFileSizeBytes 50MiB / customPatterns锛屽吋瀹规棫 `customIgnorePatterns` 鍚堝苟锛夛紱璁剧疆椤垫柊澧為粯璁ょ被鍒紑鍏炽€佸ぇ灏忎笂闄愯緭鍏ャ€佽嚜瀹氫箟妯″紡缂栬緫鍣ㄤ笌銆岄瑙堟帓闄ょ粨鏋溿€嶆寜閽紙鎸夌被鍒仛鍚堝睍绀恒€乻amples 涓婇檺 50銆佸懡涓?reason/rule/source銆佽鎺掗櫎鐩綍鏈夌晫閬嶅巻 2000 椤逛笖 `complete=false` 鏍囪锛夛紱鏂板 `checkpoint.previewExclusions` / `checkpoint.getExclusionProfiles` handler锛涢厤缃牎楠屾嫆缁濈┖妯″紡銆佺粷瀵硅矾寰勶紙鐩樼/UNC锛夈€佺函 `!`銆乣..` 瓒婄晫銆佹崲琛屾敞鍏ャ€佹湭鐭ョ被鍒?id 涓庨潪鏈夐檺鏁板€?  - 鎺掗櫎璇箟缁嗚妭锛歸in32 涓嬪己鍒舵帓闄ょ粷瀵硅矾寰勬瘮杈冪粺涓€灏忓啓褰掍竴锛堥槻 `C:\Proj` vs `c:\proj` 澶у皬鍐欐紡鎺掞級锛涖€岄噸鏂扮撼鍏ョ洰褰曠被鍒笅鏂囦欢闇€鍚屾椂鍚﹀畾鐩綍鏈韩锛堝 `!data/ + !data/keep.txt`锛夈€嶅湪璁剧疆椤电粰鍑烘彁绀猴紱鑷畾涔夋ā寮忎笉鍐嶈宓屽 `.gitignore` 瑕嗙洊锛堣 Fixed锛?
### Fixed
  - 瀛樻。鎭㈠杈圭晫鏄惧紡鍖栵細鍙垹闄ょ洰鏍囧揩鐓?`fileHashes` 涓褰曡繃鐨勮矾寰勶紙#29 璇箟锛夛紝蹇収鍚庢柊寤恒€佸揩鐓ф椂琚拷鐣?鏈浠斤紙澶嶅埗澶辫触銆佸ぇ灏忚秴闄愩€佷笉鍙锛夌殑鏂囦欢鎭㈠鏃朵笉浼氳闈欓粯鍒犻櫎锛涙仮澶?鍒涘缓鏈熼棿涓庡啓宸ュ叿浜掓枼锛岄伩鍏嶅揩鐓т笌鏂囦欢鍐欏叆绔炴€?  - 淇鎵归噺鍒犻櫎澧為噺閾炬柇閾撅紙CP-05锛夛細鏃у疄鐜板彧妫€鏌ャ€岃淇濈暀妫€鏌ョ偣鐩存帴寮曠敤銆嶄竴灞傦紝閾?A鈫払鈫扖 鍒犻櫎 {A,B} 鏃?A 琚垹鑰?B 淇濈暀锛孊 鎭㈠鏃舵柇閾撅紱鐜板湪浠庢墍鏈変繚鐣欒妭鐐瑰悜鍓嶉亶鍘嗗畬鏁寸鍏堥摼锛岃鐩存帴鎴栭棿鎺ヤ緷璧栫殑绁栧厛鍏ㄩ儴寮哄埗淇濈暀骞惰繑鍥?rejectedIds
  - 瀛樻。鍒犻櫎/鍚堝苟鎿嶄綔鎺ュ叆宸ヤ綔鍖虹骇浜掓枼閿侊紙CP-03锛夛細鍒犻櫎銆佹寜绱㈠紩鍒犻櫎銆佹壒閲忓垹闄や笌鍒涘缓/鎭㈠浜掓枼锛沗CheckpointOperationLock` 鏀寔鍚?owner 鐩稿悓宸ヤ綔鍖洪泦鍚堢殑鍙噸鍏ワ紙寮曠敤璁℃暟锛夛紝create 閿佸唴娓呯悊鏃у瓨妗ｏ紙cleanupOldCheckpoints 鈫?deleteCheckpoint锛変笉鍐嶅祵濂楃瓑寰呰嚜宸辫€屾閿?  - 淇鍥炴。骞堕噸璇?/ 鍥炴。骞跺垹闄ゆ椂鍚庣鍒犻櫎澶辫触浠嶇户缁墽琛岋紙CP-11锛夛細鍓嶇 `deleteMessage` 杩斿洖澶辫触鎴栨姏閿欐椂涓鍚庣画閲嶈瘯/娴佺▼骞跺睍绀烘槑纭敊璇紝涓嶅啀鍑虹幇銆屽墠绔凡鎴柇鑰屽悗绔巻鍙叉畫鐣欍€嶇殑闈欓粯涓嶄竴鑷达紱鍚庣鍚勫垹闄よ矾寰勭殑纾佺洏鍒犻櫎澶辫触涓嶅啀瀹屽叏闈欓粯锛岃褰?warn 渚夸簬鎺掓煡锛堝厓鏁版嵁宸叉纭Щ闄わ紝娈嬬暀鐩綍涓哄鍎跨洰褰曚笉褰卞搷姝ｇ‘鎬э級
  - 鎭㈠缁撴灉鏂板鏈浠芥枃浠舵彁绀猴紙CP-08/CP-10锛夛細`restoreCheckpoint` 杩斿洖 `unbackedPaths`锛堝揩鐓ф椂澶у皬瓒呴檺/涓嶅彲璇?澶嶅埗澶辫触鐨勬枃浠讹紝鎭㈠鏃跺彈淇濇姢涓嶄細琚垹闄わ級锛屽墠绔仮澶嶇‘璁ゅ悗灞曠ず澶辫触/閮ㄥ垎澶辫触/鏈浠芥枃浠舵竻鍗曪紱璁剧疆椤垫壒閲忓垹闄ゅ睍绀鸿渚濊禆淇濈暀涓庡垹闄ゅけ璐ョ殑鏁伴噺
  - 鏂板鎭㈠棰勮纭娴佺▼锛圕P-09锛夛細鎵€鏈夋仮澶嶅叆鍙ｏ紙鏅€氭仮澶?/ 鍥炴。骞堕噸璇?/ 鍥炴。骞跺垹闄?/ 鍥炴。骞剁紪杈戯級鍏堣皟鐢?`checkpoint.previewRestore` 璁＄畻鎭㈠璁″垝锛堝皢鎭㈠/鍒犻櫎/璺宠繃鏁伴噺 + 寰呭垹闄ゆ枃浠舵竻鍗曪級锛岀‘璁ゆ灞曠ず娓呭崟鍚庢墠鎵ц鐪熸鎭㈠锛涘緟鍒犻櫎鏂囦欢鍖哄垎銆屽揩鐓ц褰曡繃銆嶏紙#29 鐧藉悕鍗曪級涓庛€屽揩鐓у悗鏂板缓銆嶏紙榛樿淇濈暀锛岀敤鎴风‘璁ゅ悗涓€骞跺垹闄わ紝瀹炵幇銆屾挙閿€宸ュ叿鏂板缓鏂囦欢銆嶈涔夛級锛沗RestoreEngine` 鎻愬彇绾绠楀嚱鏁?`computeRestorePlan`锛岄瑙堟竻鍗曚笌瀹為檯鎵ц鐨勫垹闄や弗鏍间竴鑷达紱`restoreCheckpoint` 鏂板 `deleteUntrackedFiles` 閫夐」锛屾湭纭鏃朵繚鎸佸師鏈変繚鎶よ涔変笉鍒犻櫎浠讳綍蹇収鍚庢柊寤烘枃浠?  - 鎭㈠纭娴佺▼瀹夊叏鍔犲浐锛歚restoreAndRetry` / `restoreAndDelete` / `restoreAndEdit` 鐨勩€屽垹闄ゅ揩鐓у悗鏂板缓鏂囦欢銆嶆敼涓鸿皟鐢ㄦ柟锛堢‘璁ゆ锛夋樉寮忎紶鍙傜‘璁わ紝榛樿 false鈥斺€旂粫杩囩‘璁ゆ鐨勮皟鐢ㄤ笉浼氶潤榛樺垹闄ゆ枃浠讹紱鍥炴。涓夎繛涓?`deleteMessage` 澶辫触鏃朵笉鍐嶅彧鎻愮ず锛岃€屾槸閲嶆柊鍔犺浇鍘嗗彶鎷夊洖鍓嶇绐楀彛涓庡悗绔竴鑷达紱鎭㈠纭妗嗗彇娑堟椂娓呯悊鏆傚瓨鐨勯瑙堝姩浣滅姸鎬侊紱棰勮鏈熼棿鎭㈠鎸夐挳鏄剧ず loading 骞剁鐢?  - 瀛樻。缁存姢鏀硅繘锛歚pruneMissingBackupCheckpointRecords` 椤哄甫娓呯悊瀛ゅ効澶囦唤鐩綍锛堢鐩樺瓨鍦ㄤ絾鏃犱换浣曡褰曞紩鐢紝濡傚垹闄ゅけ璐?宕╂簝娈嬬暀锛屼粎澶勭悊 `cp_*` 鏍煎紡鐩綍锛夛紱璁剧疆椤靛瓨妗ｈ鎯呭睍绀恒€孨 涓枃浠舵湭澶囦唤銆嶏紙鎮仠鏄剧ず璺緞娓呭崟锛屽幓闄ゅ伐浣滃尯浣滅敤鍩熷墠缂€锛夛紱`CheckpointOperationLock` 鍙噸鍏ユ斁瀹戒负銆岃姹傞泦鍚堟槸宸叉寔鏈夐泦鍚堢殑瀛愰泦鍗虫斁琛屻€嶏紝宓屽璋冪敤鏇翠笉鏄撴閿?  - 娣卞害瀹℃煡淇锛氬揩鐓у己鍒舵帓闄よ寖鍥翠粠瀛樻。鐩綍鎵╁ぇ涓烘暣涓墿灞曞瓨鍌ㄦ牴锛堣嚜瀹氫箟鏁版嵁鐩綍浣嶄簬宸ヤ綔鍖哄唴鏃讹紝memory/conversations 绛夋墿灞曟暟鎹笉鍐嶈繘鍏ュ瓨妗ｏ級锛涙仮澶嶆椂銆屽垹闄ゅ浣欑┖鐩綍銆嶇撼鍏?`deleteUntrackedFiles` 纭鎺у埗锛堝揩鐓у悗鏂板缓鐨勭┖鐩綍榛樿淇濈暀锛?29 璇箟锛岄瑙堟竻鍗曚竴骞跺睍绀猴級锛沴egacy 瀛樻。棰勮杩斿洖 `legacy` 鏍囪锛屽墠绔睍绀恒€屾仮澶嶄互澶囦唤鍐呭涓哄噯銆嶉伩鍏嶈鍒ゃ€屾棤鍙樻洿銆嶏紱鎭㈠纭妗嗘墦寮€鏈熼棿鎭㈠鎸夐挳绂佺敤锛岄槻姝㈤噸澶嶇偣鍑昏鐩栫‘璁ゅ唴瀹?  - 淇澧為噺閾炬仮澶嶇储寮曢敊璇細澧為噺鑺傜偣纾佺洏涓婂彧淇濆瓨 `changes` 閲岀殑鏂囦欢锛屼絾鎭㈠绱㈠紩姝ゅ墠鎶婅鑺傜偣瀹屾暣 `fileHashes` 閮芥寚鍚戝叾鐩綍锛屽鑷存湭鍙樺寲鏂囦欢鎭㈠鏃舵姤 `missing_in_chain`锛涚幇鍦ㄦ寜 `changes` 闄愬畾鑺傜偣澶囦唤杈圭晫锛屾湭鍙樺寲鏂囦欢浠庢洿鏃╄妭鐐癸紙base锛夋仮澶?  - 淇瀛樻。璺緞瀹夊叏缂哄彛锛氬浠芥簮鐩綍锛坄backupDir`锛夋潵鑷瓨妗ｅ厓鏁版嵁锛屾崯鍧忔暟鎹彲鍚?`..`/缁濆璺緞锛岀幇鍦ㄦ仮澶嶆椂鏍￠獙澶囦唤婧愬繀椤讳綅浜庡瓨妗ｆ牴鐩綍鍐咃紙瓒婄晫瑙嗕负閾句笂缂哄け锛夛紱鎭㈠鐩爣璺緞鍏ㄧ▼鍋氱鍙烽摼鎺?junction 妫€鏌ワ紙`resolveSafePathInsideRoot`锛夛紝閾炬帴涓嶈兘缁曡繃宸ヤ綔鍖鸿竟鐣?  - 淇鏃х増瀛樻。锛堟棤 `fileHashes`锛夋仮澶嶅畨鍏ㄩ殣鎮ｏ細澶氭牴宸ヤ綔鍖轰笅鏄庣‘鎷掔粷锛堟棫璁板綍鏃犲伐浣滃尯韬唤锛屾棤娉曠‘瀹氭枃浠跺綊灞烇級锛涘崟鏍规仮澶嶄互澶囦唤鐩綍瀹為檯鍐呭涓虹洰鏍囥€佺粷涓嶅垹闄ゅ綋鍓嶅伐浣滃尯浠讳綍鏂囦欢锛堟棫璁板綍娌℃湁鈥滃揩鐓ф椂鍙鈥濇竻鍗曪級锛屾浛浠ｆ鍓嶁€滃垹闄ゅ浠介噷娌℃湁鐨勬墍鏈夋枃浠垛€濈殑鍗遍櫓琛屼负
  - 淇鍚屼竴瀵硅瘽鍒囨崲宸ヤ綔鍖哄悗澧為噺閾鹃敊涔憋細鏂板瓨妗ｈ瘑鍒埌宸ヤ綔鍖烘寚绾逛笉涓€鑷存椂浠庢柊鐨勫畬鏁村浠藉紑濮嬶紙鏂紑鏃ч摼锛夛紝涓嶅啀璺ㄥ伐浣滃尯涓叉帴澧為噺
  - 閮ㄥ垎鎭㈠澶辫触鏃惰繑鍥?`error` 鎽樿锛堝墠 5 鏉¤矾寰?鍘熷洜锛岃秴鍑鸿鏁帮級锛屽墠绔?`restoreCheckpoint` 绫诲瀷鍚屾琛ュ厖 `failures` 瀛楁
  - 淇娴佸紡鎶ラ敊鍚庨噸璇曟畫鐣欏崐鎴洖绛旓細娴佸紡杩囩▼涓悗绔姤閿欐椂锛屽悗绔笉浼氭寔涔呭寲鍗婃埅 assistant 娑堟伅锛屼絾鍓嶇绐楀彛浼氫繚鐣欐湁鍐呭鐨勫崐鎴秷鎭紝鐐瑰嚮閿欒閫氱煡涓婄殑銆岄噸璇曘€嶏紙`retryAfterError`锛変箣鍓嶄笉浼氭竻鐞嗭紝瀵艰嚧閲嶈瘯鍚庣獥鍙?鍘嗗彶鍑虹幇鍗婃埅鍥炵瓟娈嬬暀锛屼笖涓庡悗绔巻鍙查敊浣嶏紱鐜板湪 `handleError` 浼氳褰曞け璐ュ崐鎴秷鎭?ID锛宍retryAfterError` 閲嶈瘯鍓嶅洖婊氾紙鍒犻櫎绐楀彛娑堟伅 + 娓呯悊妫€鏌ョ偣 + 闃插尽鎬у悓姝ュ垹闄ゅ悗绔級锛岄敊璇潯銆屽叧闂€嶆寜閽笌鍙戦€佹柊娑堟伅涔熶細涓€骞舵竻鐞嗗け璐ユ畫鐣欙紝宸ュ叿鍝嶅簲鍚庣殑銆岀户缁璇濄€嶈涔変笉鍙楀奖鍝嶏紙涓嶅垹闄ゆ甯稿巻鍙诧級锛涙柊澧炲洖褰掓祴璇?12 鐢ㄤ緥锛坒rontend `streamErrorRetry.test.ts`锛?  - 淇鍔ㄦ€佷笂涓嬫枃绛栫暐閫夐」鐨勮瀵兼€ф嫭鍙锋爣娉細鍗曚唤妯″紡閫夐」鍘熷厛甯︽湁銆岋紙褰撳墠绛栫暐锛夈€嶏紙zh-CN锛夋垨銆岋紙褰撳墠琛屼负锛夈€嶏紙en/ja锛夋敞閲婏紝鑰岃閫夐」涓庝繚鐣欐ā寮忔槸骞跺垪鍙垏鎹㈢殑锛屾爣娉ㄣ€屽綋鍓嶃€嶄細璁╃敤鎴峰湪鍒囨崲鍚庣湅鍒伴敊璇殑褰掑睘璇箟锛涚幇鍦ㄧЩ闄ゆ嫭鍙锋敞閲婏紝涓夎缁熶竴涓轰腑鎬ф枃妗堛€屽崟浠藉姩鎬佷笂涓嬫枃 / Single dynamic context / 鍗樹竴銇嫊鐨勩偝銉炽儐銈偣銉堛€?  - 淇鍔ㄦ€佷笂涓嬫枃绛栫暐璀﹀憡鏂囨纭紪鐮佽嫳鏂囷細璁剧疆椤甸€変腑淇濈暀妯″紡鏃舵彁绀恒€宲reserve 浼氭妸鏃у洖鍚堢殑鍔ㄦ€佸揩鐓у浐瀹氭彃鍥炲師浣嶁€︺€嶏紝鑰岄€夐」鍦?UI 涓婃樉绀虹殑鏄腑鏂囥€屼繚鐣欐棫鍔ㄦ€佷笂涓嬫枃鍘熶綅銆嶏紝鐢ㄦ埛鏃犳硶鎶婁簩鑰呭搴旓紱鐜板湪鍙ラ鐩存帴浣跨敤閫夐」鐨勪腑鏂囧悕绉帮紝涓嶅啀鍑虹幇鑻辨枃鏍囪瘑
  - 淇绌烘彁绀鸿瘝淇濆瓨鍚庡洖閫€榛樿妯℃澘锛圥R #5锛夛細`resolvedMode?.template || promptConfig?.template`锛圥romptManager.ts锛変笌 `mode?.template || ...`锛圫ettingsManager.getSystemPromptTemplate锛夌殑绌哄瓧绗︿覆鍥為€€瀵艰嚧 legacy 妯″紡鏄惧紡淇濆瓨鐨勭┖妯℃澘鍦ㄨ繍琛屾椂琚叏灞€妯℃澘瑕嗙洊锛岀敤鎴锋棤娉曠湡姝ｆ竻绌烘ā鏉匡紱涓ゅ鏀逛负 `??`锛坣ullish coalescing锛夛紝绌哄瓧绗︿覆鍘熸牱淇濈暀锛涘墠绔?`loadModeConfig` 鍚屾鐢?`typeof === 'string'` 鍒ゆ柇鑰岄潪 `||` 鍏滃簳锛岀┖妯℃澘涓嶅啀琚?DEFAULT_TEMPLATE 椤舵浛锛屾柊澧炲洖褰掓祴璇曢攣瀹氳琛屼负
  - 淇鎻愮ず璇嶅鍑烘祦绋嬩笉鍙帶锛圥R #5锛夛細鍓嶇 Blob 涓嬭浇鍦?webview 鐜淇濆瓨浣嶇疆涓庢枃浠跺悕涓嶅彈鐢ㄦ埛鎺у埗锛屾敼涓洪€氳繃鏂板鐨?`exportPromptModes` webview handler 璋冪敤 `vscode.window.showSaveDialog` 璁╃敤鎴烽€夋嫨淇濆瓨浣嶇疆锛岀‘璁ゅ悗鎵嶅啓鏂囦欢锛涘彇娑堜繚瀛樺璇濇鏃朵笉鍐欐枃浠跺苟杩斿洖 `{ success: false, cancelled: true }`锛屾垚鍔熷悗鎵嶆姤鍛婃垚鍔燂紱瀵煎嚭鎴愬姛 / 鍙栨秷 / 澶辫触鍧囨湁鏄庣‘鍙嶉锛屼笉鍐嶅嚭鐜般€屽凡瀵煎嚭浣嗕笉鐭ラ亾瀛樺埌鍝€?  - 淇鎭㈠璺緞鏈帴鍏ュ洓灞傛帓闄ゆā鍨嬶紙瀹℃煡 H-1锛夛細鎭㈠鏃剁殑鐩爣鐘舵€佽繃婊や笌褰撳墠宸ヤ綔鍖虹姸鎬佹敹闆嗘鍓嶅彧璧?`.gitignore` + 鏃?`customIgnorePatterns` 涓ゅ眰锛屾仮澶嶅彲鑳芥妸鏂囦欢鍐欏洖褰撳墠鏄庣‘鎺掗櫎鐨勮矾寰勶紙`dist/`銆乣data/`銆佽秴杩囧ぇ灏忎笂闄愮殑鏂囦欢锛夋垨鍒犻櫎褰撳墠搴旀帓闄ょ殑鏂囦欢锛涚幇鍦?`createIgnoreResolver` 缁熶竴鏋勯€犲畬鏁村洓灞傝鍒欙紙寮哄埗鎺掗櫎鍚墿灞曞瓨鍌ㄦ牴銆侀粯璁ょ被鍒€佸祵濂?`.gitignore`銆佹柊鏃ц嚜瀹氫箟妯″紡鍚堝苟锛夛紝`filterRestoreTargetScoped` / `collectCurrentWorkspaceState` 涓庡揩鐓ф瀯寤哄悓涓€鍙ｅ緞锛屾仮澶嶄笉鍐嶈Е纰板綋鍓嶆槑纭帓闄ょ殑璺緞
  - 淇绌哄閲忚妭鐐规仮澶嶅繀澶辫触锛堝鏌?H1锛夛細澧為噺閾剧储寮曟瀯寤烘椂 `changes` 涓虹┖鏁扮粍鐨勮妭鐐癸紙宸ュ叿鎵ц浣嗘棤鏂囦欢鍙樺寲鐨?before/after 瀛樻。锛夎褰撲綔瀹屾暣鑺傜偣锛屽叾鍏ㄩ儴鍝堝笇琚寚鍒拌嚜宸辩殑绌哄浠界洰褰曞苟瑕嗙洊鏇存棭鑺傜偣锛屾仮澶嶄换浣曟紓绉绘枃浠堕兘鎶?`missing_in_chain`锛涚幇鍦ㄥ尯鍒嗐€屾湭鎻愪緵 changes锛堝畬鏁磋妭鐐癸級銆嶄笌銆岀┖鏁扮粍锛堢┖澧為噺鑺傜偣锛屼笉绱㈠紩浠讳綍鏂囦欢锛夈€嶏紝绌哄閲忓悗鐨勬仮澶嶇敱鏇存棭鑺傜偣鎻愪緵鏂囦欢
  - 淇璺ㄦ牸寮忓悎骞舵柇閾撅紙瀹℃煡 M6锛夛細瀹归噺娓呯悊鎶?legacy 甯冨眬锛坄cp_xxx/relative`锛夎娓呯悊鑺傜偣骞跺叆鏂版牸寮忓悗缁ф椂锛屾枃浠惰鍘熸牱澶嶅埗鍒板悗缁ф牴鐩綍涓斾笉鍐欒繘 manifest锛屾仮澶嶆椂 `missing_in_chain`锛涚幇鍦ㄦ寜鍚庣户 scoped 甯冨眬閲嶅啓璺緞锛坄ws_xxx/` 鍓嶇紑锛夊啀澶嶅埗锛屽苟鎶婂悎骞舵枃浠剁殑 scoped 閿苟鍏ュ悗缁?manifest.files锛坔ash 鍙栬嚜琚垹鑺傜偣锛夛紝legacy鈫掓柊鏍煎紡杩囨浮鏈熸竻鐞嗕笉鍐嶇牬鍧忓彲鎭㈠鎬?  - 淇瀛樻。鍙栨秷绔炴€佷笌閿佺瓑寰呰８寮傚父锛堝鏌?M4/M5锛夛細鍙栨秷鍙戠敓鍦ㄧ瓑寰呮枃浠跺啓閿佹湡闂存椂锛岄攣绠＄悊鍣ㄦ姏鏅€?Error 浠?`runExclusive` 婕忓嚭骞跺啋娉″埌宸ュ叿寰幆涓柇鏁磋疆鎵ц锛涚幇鍦?create/restore/deleteAll/deleteBatch 澶栧眰鎹曡幏閿佸彇娑堥敊璇浆涓哄彇娑堢粨鏋滐紱鍙栨秷鍙戠敓鍦ㄥ鍒跺畬鎴愩€佸厓鏁版嵁钀界洏涔嬪墠鏃舵鏌ョ偣浠嶄細琚繚瀛樹笖杩涘害琚?`done` 瑕嗙洊鈥斺€旂幇鍦ㄥ啓 manifest 涓庡啓浼氳瘽鍏冩暟鎹墠妫€鏌?`throwIfAborted`锛岃繘搴︾粓鎬佹敼涓?`signal.aborted ? 'cancelled' : 'done'`
  - 淇鎺掗櫎瑙勫垯瀵规瘮婕忕被鍒紑鍏筹紙瀹℃煡 M3/M-4锛夛細`rulesChanged` 鍙瘮杈冨ぇ灏忎笂闄愪笌鑷畾涔夋ā寮忥紝璁剧疆椤典粎寮€鍏抽粯璁ょ被鍒紙濡傚叧闂?logs锛夋椂鎭㈠璇存槑璇姤銆岃鍒欐湭鍙樺寲銆嶏紱鐜板湪鍚屾椂姣旇緝閿帓搴忓悗鐨?`enabledProfiles` 涓庤鍒欑増鏈彿
  - 淇鑷畾涔夋帓闄ゆā寮忚宓屽 `.gitignore` 鍙屽悜瑕嗙洊锛堝鏌?M-1锛夛細鑷畾涔夋ā寮忎笌榛樿绫诲埆鍘熷厛娉ㄥ叆鏍逛綔鐢ㄥ煙 matcher锛屽祵濂?`.gitignore` 鍙惁瀹氳嚜瀹氫箟灞傜殑 `!` 瑙勫垯鎴栧弽杩囨潵锛涚幇鍦ㄨ嚜瀹氫箟妯″紡浠庝綔鐢ㄥ煙閾炬媶鍑猴紝浣滀负鎵€鏈変綔鐢ㄥ煙姹傚€间箣鍚庣殑鐙珛鏈€缁堥樁娈碉紙寮哄埗鎺掗櫎浠嶄笉鍙鐩栵級锛屻€岃缃〉瑙勫垯鏈€鍚庣敓鏁堛€嶄笌璁″垝璇箟涓€鑷?  - 淇鎺掗櫎棰勮 `complete` 璋庢姤锛堝鏌?M-6锛夛細涓绘壂鎻忛亣鍒颁笉鍙鐩綍鏃堕潤榛樿烦杩囦笖 `complete` 浠嶄负 true锛涚幇鍦ㄤ笉鍙鐩綍浜у嚭 `unreadable` 鎺掗櫎鏉＄洰骞剁疆 `complete=false` 璁″叆缁熻
  - 淇鎭㈠璇存槑涓庢湭澶囦唤鎻愮ず鍥炲綊锛歚rulesChanged` 涓?`excludedNote` 琛ュ叏鍚庢仮澶嶈鏄庡噯纭紱瀛樻。鎽樿锛坄CheckpointSummary`锛夊寲鍚庤缃〉銆孨 涓枃浠舵湭澶囦唤銆嶆彁绀轰涪澶扁€斺€旀憳瑕佽ˉ榻?`unbackedPaths` 鐩稿叧瀛楁锛屽墠绔偓鍋滄彁绀烘仮澶?  - 淇 append-only 宕╂簝鎭㈠鍚庤鏁版案涔呬笉涓€鑷达紙瀹℃煡 H1锛夛細灏炬 rename 鎴愬姛浣?index 鍐欏け璐?宕╂簝鏃舵畫鐣欒浼氬湪涓嬫杩藉姞琚苟鍏ユ璁℃暟锛宍index.totalMessages` 涓庡悇娈?count 姘镐箙鐩稿樊銆佸叏閲忚涓庡垎椤佃鍙ｅ緞涓嶄竴锛涚幇鍦ㄥ熬娈佃鍙栧悗鍏堟寜 index 鎻愪氦鐐规埅鏂紙`slice(0, count)`锛夊啀鎷兼帴鏂板唴瀹癸紝at-most-once 璇箟涓嬮噸璇曚笉閲嶅锛屽穿婧冩畫鐣欎笉浼氭硠婕?  - 淇鍏冩暟鎹枃浠舵崯鍧忓鑷村垪琛?UNKNOWN_ERROR锛堢湡瀹炴晠闅滐級锛歚saveMetadata` 鍘熶负闈炲師瀛?`writeFile`锛屽啓鍏ヤ腑閫斿穿婧?鏂數鍗虫埅鏂?`{id}.meta.json`锛堝 24MB 澶ф枃浠讹級锛岃鍙栨椂 `parse_error` 鎶涘埌鍓嶇闃诲瀵硅瘽鍒楄〃锛涚幇鍦ㄥ厓鏁版嵁鍐欏叆鏀逛负涓存椂鏂囦欢 + rename 鍘熷瓙鏇挎崲锛坕ndex 寮忔彁浜ょ偣锛夛紝`getMetadata` 閬?`parse_error` 鏃舵妸鎹熷潖鏂囦欢鏀瑰悕澶囦唤涓?`.corrupt-*`锛堝彧淇濈暀涓€浠斤級骞朵粠鍘嗗彶鏃堕棿鎴抽噸寤?fallback 鍏冩暟鎹繑鍥烇紝涓嶅啀鍚戜笂鎶涢敊锛堝瓨妗ｈ褰曞垪琛ㄩ殢鎹熷潖鏂囦欢涓㈠け灞為檷绾т唬浠凤紝纾佺洏 `cp_*` 澶囦唤鐩綍涓嶅彈褰卞搷涓斾笉浼氳鑷姩娓呯悊锛?  - 淇瀹屾暣鎬ф鏌ヤ袱绫昏鍒わ紙瀹℃煡 M1锛夛細legacy 鍗曟枃浠跺巻鍙插彧 `exists` 涓嶈В鏋愬鑷存崯鍧?JSON 鎶?`ok`鈥斺€旂幇鍦ㄨ嚦灏戝仛涓€娆?`JSON.parse` 鎺㈡祴锛泂egmented 鍒嗘敮 index 瀹屽ソ浣嗘鏂囦欢缂哄け鏃惰鎶?`ok`鈥斺€旂幇鍦ㄥ姣忎釜娈?`stat` 瀛樺湪鎬э紙涓嶈В鏋愬唴瀹癸紝淇濇寔 HIS-11 鍙缁撴瀯鐩爣锛夛紝浠讳竴缂哄け鍗?`readable=false`
  - 淇娈电紦瀛樺厓绱犲紩鐢ㄦ薄鏌擄紙瀹℃煡 M2锛夛細`loadSegmentedHistory` 鐨?`slice` 鍙鍒舵暟缁勩€佸厓绱犱笌缂撳瓨鍏变韩寮曠敤锛宍ContextTrimService` 瀵?`tokenCountByChannel` 鐨勫師鍦拌祴鍊间細姹℃煋缂撳瓨锛涚幇鍦ㄥ姞杞借矾寰勮繑鍥炲墠瀵瑰厓绱犳祬鎷疯礉锛岀紦瀛樺彧璇昏竟鐣岃惤瀹烇紱缂撳瓨閿撼鍏ユ鏂囦欢 mtime锛坄revision::m{mtime}`锛夛紝澶栭儴杩涚▼鏀瑰啓娈靛唴瀹瑰悗鍛戒腑鍓?`stat` 姣斿鑷姩澶辨晥锛堝鏌?M5锛?  - 淇鐢ㄩ噺涓庡厓鏁版嵁涓€鑷存€э細`updateSummary` 涓嶅啀鏃犳潯浠跺啓鍓嶇浼犳潵鐨?`messageCount`鈥斺€旇秴杩囧巻鍙插疄闄呮潯鏁版椂鎸?`index.totalMessages` 閽冲埗锛屽墠绔?IPC 澶辫触鏃朵笉鍚屾鏈湴璁℃暟锛堝鏌?M3锛夛紱append 閬囥€宨ndex 瀛樺湪浣嗗熬娈电己澶?鎹熷潖銆嶆椂涓嶅啀姘镐箙澶辫触锛屽洖閫€鍏ㄩ噺閲嶅啓鑷剤锛堝鏌?M4锛?  - 淇鍓嶇鍘嗗彶绐楀彛涓庣紦瀛橀棶棰橈細backfill 鎶?IPC 澶辫触璇綋銆屽凡鍒板巻鍙插紑澶淬€嶇疆 `windowStartIndex=0` 瀵艰嚧鏇存棭娑堟伅鏃犳硶涓婃媺鈥斺€旂幇鍦ㄩ敊璇笌绌洪〉鍖哄垎锛岄敊璇椂鏀惧純鍚堝苟淇濈暀鍘熺獥鍙ｏ紙瀹℃煡 L2锛夛紱`loadMoreConversations` 娓告爣鎸夊疄闄呰繑鍥炴暟閲忓墠杩涳紝鎵归噺缁撴灉缂哄け鏃朵笉鍐嶈烦杩囨湭鍔犺浇瀵硅瘽锛堝鏌?L3锛夛紱娑堟伅涓棿浣嶇疆鍚岄暱搴︽浛鎹紙杩熷埌 cancelled chunk 娓呯悊锛変細鍛戒腑闄堟棫鍙缂撳瓨鈥斺€擿replaceMessageAt` 闈炲熬閮ㄦ浛鎹㈡椂娓呯紦瀛橈紙瀹℃煡 L1锛夛紱`addBatch` 鐩撮€?append 鍙兘缁曡繃 functionResponse 鍘婚噸瀹夊叏缃戔€斺€斿绾︽敞閲婃槑纭粎闄愮函杩藉姞 user/model 骞跺 functionResponse 鏄惧紡鎷掔粷锛堝鏌?L4锛?  - 淇瀛恆gent 缁窇涓㈠け provider 缂撳瓨锛堢敤鎴峰弽棣堬級锛氬瓙agent 姣忔鎵ц鍒嗛厤鏂?runId 浣滀负璇锋眰 `conversationId`锛孌eepSeek/Anthropic 鐨?`user_id` 鎸夊畠鍝堝笇锛宍continueFromRunId` 缁窇鏃舵柊 runId 钀藉叆鏂扮紦瀛樺煙銆佸墠缂€缂撳瓨蹇?miss锛涚幇鍦ㄧ画璺戞椂 `conversationId` 鐩存帴娌跨敤鏃?run 鐨?runId锛坄request.continueFromRunId || runId`锛夛紝`user_id` 鍝堝笇杈撳叆涓庢棫 run 瀹屽叏涓€鑷淬€佺紦瀛樺煙澶╃劧鐩稿悓鈥斺€旀ā鍨嬭皟鐢ㄥ伐鍏峰彧闇€浼?`continueFromRunId`锛岀郴缁熻嚜鍔ㄦ敞鍏ョǔ瀹氱紦瀛樺煙锛屾棤闇€浠讳綍棰濆瀛楁锛堝悓鏃剁Щ闄や复鏃跺紩鍏ョ殑 `GenerateRequest.cacheDomainId` 鏈哄埗锛宖ormatter 鍥為€€涓?conversationId 鍗曚竴鏉ユ簮锛?  - 淇瀛恆gent 鍙嶅璋冪敤 todo 宸ュ叿鎶ャ€屾棤鏉冮檺銆嶏紙鐢ㄦ埛鍙嶉锛夛細`todo_write`/`todo_update` 渚濊禆涓讳細璇?`ToolContext.conversationId`锛屽瓙agent 鎵ц璺緞涓嶆敞鍏ヨ鍊硷紝澹版槑浜嗕篃蹇呯劧澶辫触骞舵氮璐硅凯浠ｂ€斺€旂幇鍦ㄤ粠瀛恆gent 宸ュ叿澹版槑涓庢墽琛屾湡鍏佽鍒楄〃缁熶竴鎺掗櫎 todo 宸ュ叿锛堜富浼氳瘽涓嶅彈褰卞搷锛?  - 淇瀛恆gent 绐楀彛琚熬閮ㄦ牎鍑嗘竻绌猴紙鐢ㄦ埛鍙嶉锛夛細SubAgentMonitor 鏀跺埌宸ュ叿/鍐呭浜嬩欢鏃跺鑱氱劍 run 鎷夊彇灏鹃儴 20 鏉″苟鏁翠綋鏇挎崲绐楀彛锛岀敤鎴枫€屽姞杞芥洿鏃╂秷鎭€峱repend 鐨勫巻鍙茶娓呯┖锛涚幇鍦ㄥ熬閮ㄦ牎鍑嗘敼涓轰繚鐣欐棭浜庝紶鍏?`startIndex` 鐨勫墠缂€鍚堝苟锛坄replaceRunContentWindowPreservingPrefix`锛夛紝鍒犻櫎/閲嶈瘯鐨勬潈濞佹牎鍑嗚矾寰勪粛璧扮函鏇挎崲
  - 淇瀛恆gent 鍐欐枃浠跺啿绐佹彁绀轰笉鍙嬪ソ锛堢敤鎴峰弽棣堬級锛氬啿绐佹秷鎭笉鍐嶇缁熴€屼笉瑕侀噸璇曘€嶏紝鏀逛负鏄庣‘鎸佹湁鑰呰韩浠斤紙`agent "X"` / 涓讳細璇?/ 瀛樻。鎿嶄綔锛変笌涓夋鎸囧紩锛堜笉寰幆閲嶈瘯 鈫?鍏堝仛鍏朵粬宸ヤ綔銆佹寔鏈夎€呴噴鏀惧悗閲嶈瘯 鈫?鎸佺画鍐茬獊鍦ㄦ渶缁堝洖澶嶄笂鎶ヤ富浼氳瘽鍗忚皟锛夛紝`presets.ts` 鎸囧紩璇箟鍚屾
  - 淇瀛恆gent 杩愯鏃堕棿鏄剧ず鏈湴缁濆鏃堕棿鎴筹紙鐢ㄦ埛鍙嶉锛夛細SubAgentMonitor 鐨勩€岃繍琛屼腑 路 15:16:46銆嶆敼涓虹浉瀵硅€楁椂锛坄42s` / `2m30s` / `1h5m`锛屾椿璺冩€佹樉绀哄凡杩愯鏃堕暱銆佺粓鎬佹樉绀烘€昏€楁椂锛夛紝鏂板 1s ticker 浠呭湪鏈夋椿璺?run 鏃惰繍琛?
### Added
  - 鏂板 `exportPromptModes` webview handler锛坄webview/handlers/SettingsHandlers.ts`锛変笌鍥炲綊娴嬭瘯 `backend/__tests__/webview/promptModeExport.test.ts`锛堝彇娑堝璇濇涓嶅啓鏂囦欢 / 閫夎矾寰勫苟鍐欐枃浠舵垚鍔熷悗鎵嶅搷搴旓紝2 鐢ㄤ緥锛夛紱`PromptManager.promptEntries.test.ts` 鏂板銆屾樉寮忕┖妯℃澘涓嶅洖閫€鍏ㄥ眬妯℃澘銆嶇敤渚嬶紱vscode mock 琛ュ厖 `showSaveDialog` / `showOpenDialog`
  - 鏂板瀛樻。蹇収鏋勫缓鍣?`CheckpointSnapshotBuilder`锛堝鏍瑰伐浣滃尯鎵弿銆佸己鍒舵帓闄ょ粷瀵硅矾寰勯槻瀛樻。鑷浠姐€佸崟鏂囦欢澶у皬涓婇檺璁板綍鎺掗櫎鍘熷洜銆佹祦寮忓搱甯屻€佹湁鐣屽苟鍙戯級涓庡崟鍏冩祴璇?7 鐢ㄤ緥
  - 鏂板瀛樻。鎭㈠寮曟搸 `CheckpointRestoreEngine`锛堝閲忛摼鏂囦欢绱㈠紩 O(1) 鏌ヨ銆乻coped 璺緞瀹夊叏瑙ｆ瀽鍏煎鏃х浉瀵硅矾寰勫瓨妗ｃ€佸搱甯屾牎楠屻€佸け璐ユ竻鍗曞尯鍒?missing_in_chain / hash_mismatch / copy_failed / delete_failed锛変笌鍗曞厓娴嬭瘯 6 鐢ㄤ緥
  - 鏂板宸ヤ綔鍖鸿韩浠戒笌璺緞瀹夊叏妯″潡 `CheckpointWorkspace`锛堝伐浣滃尯鏍?ID / 鎸囩汗 / 蹇収鏍￠獙 / 瀹夊叏鐩稿璺緞涓庣鍙烽摼鎺ヨ竟鐣岋級涓庡瓨妗ｆ搷浣滀簰鏂ユā鍧?`CheckpointOperationLock`锛堝伐浣滃尯绾?FIFO 浜掓枼 + 鍏ㄥ眬鏂囦欢鍐欓攣绛夊緟锛夛紝閰嶅鍗曞厓娴嬭瘯 26 鐢ㄤ緥
  - 鏂板瀛樻。宸ヤ綔鍖鸿竟鐣岄泦鎴愭祴璇?`CheckpointManagerWorkspace.test.ts` 6 鐢ㄤ緥锛堝鏍瑰垱寤?鎭㈠銆佽法椤圭洰鎭㈠鎷掔粷銆佸瓨妗ｇ洰褰曚綅浜庡伐浣滃尯鍐呮椂鑷帓闄ゃ€佸鏍逛笅鏃у瓨妗ｆ槑纭嫆缁濄€乽nbacked 鏂囦欢鎭㈠淇濇姢锛?  - 鏂板澧為噺閾?璺緞瀹夊叏鍥炲綊娴嬭瘯 6 鐢ㄤ緥锛氬閲忚妭鐐规湭鍙樺寲鏂囦欢浠?base 鎭㈠锛坄changes` 杈圭晫锛夈€乣backupDir` 瓒婄晫澶囦唤婧愬拷鐣ャ€佽法宸ヤ綔鍖哄垱寤烘柊瀛樻。鏂摼銆佹棤 `fileHashes` 鏃у瓨妗ｅ鏍规嫆缁濄€佹棫瀛樻。鎭㈠涓嶅垹闄ゅ浠藉鏂囦欢銆侀儴鍒嗗け璐ヨ繑鍥為敊璇憳瑕?  - 鏂板娴嬭瘯锛歚CheckpointOperationLock` 鍙噸鍏?3 鐢ㄤ緥锛堝悓 owner 宓屽涓嶆閿併€佷笉鍚?owner 浠嶄覆琛屻€佸唴灞傞噴鏀句笉鎻愬墠閲婃斁澶栧眰閿侊級銆佹壒閲忓垹闄ょ鍏堥棴鍖?1 鐢ㄤ緥锛堜繚鐣欏熬鑺傜偣淇濇姢鏁存潯鍩洪摼锛夈€佹仮澶嶈繑鍥?`unbackedPaths` 涓旀湭澶囦唤鏂囦欢涓嶅彈鍒犻櫎 1 鐢ㄤ緥
  - 鏂板娴嬭瘯锛歚computeRestorePlan` 鐧藉悕鍗?鍙椾繚鎶?蹇収鍚庢柊寤烘枃浠惰繃婊?1 鐢ㄤ緥銆乣previewRestore` 棰勮鏃犲壇浣滅敤 + 鏈‘璁や笉鍒犲揩鐓у悗鏂板缓鏂囦欢 + 纭鍚庡垹闄?1 鐢ㄤ緥
  - 鏂板娴嬭瘯锛氶攣瀛愰泦鍙噸鍏?1 鐢ㄤ緥銆佸鍎垮浠界洰褰曟竻鐞?1 鐢ㄤ緥銆佸墠绔?`checkpointActions.test.ts` 9 鐢ㄤ緥锛坧reviewRestore 閫忎紶/寮傚父銆乺estoreCheckpoint 纭鏍囪銆乺estoreAndRetry 鍒犻櫎澶辫触涓 + 閲嶈浇鍘嗗彶銆佹垚鍔熻矾寰勩€乺estoreAndDelete 澶辫触閲嶈浇鍘嗗彶锛?  - 鏂板娴嬭瘯锛氭墿灞曞瓨鍌ㄦ暣鏍规帓闄わ紙memory 鏁版嵁涓嶈繘瀛樻。锛夊寮?1 鐢ㄤ緥銆佸揩鐓у悗绌虹洰褰曢粯璁や繚鐣?+ 纭鍚庢竻鐞?1 鐢ㄤ緥
  - 璁剧疆椤靛瓨鍌ㄨ矾寰勫尯鍩熸柊澧炪€屾墦寮€鐩綍銆嶆寜閽紝澶嶇敤鏃㈡湁 `storagePath.openInExplorer` webview handler锛屼竴閿湪绯荤粺鏂囦欢绠＄悊鍣ㄤ腑鎵撳紑褰撳墠鐢熸晥鐨勫瓨鍌ㄧ洰褰?  - 鏂板 checkpoint 妯″潡鍏变韩绫诲瀷濂戠害 `backend/modules/checkpoint/types.ts`锛坄CheckpointExcludeReason` / `CheckpointExcludedEntry` / `CheckpointExclusionSummary` / `CheckpointExclusionConfig` / `CheckpointIgnoreSnapshot` / `CheckpointManifest` / `CheckpointSummary` / `CheckpointExclusionPreviewResult` / `CheckpointOperationProgress`锛夛紝浣滀负鎺掗櫎涓?manifest 骞惰鏀归€犵殑鎺ュ彛閿氱偣
  - 鏂板榛樿鎺掗櫎绫诲埆妯″潡 `CheckpointExclusionProfiles.ts`锛? 涓被鍒紙logs / aiModels / datasets / caches / pythonVenvs / buildArtifacts / largeMedia / archives锛変弗鏍兼寜璁″垝娓呭崟锛堜笉鍚?`*.bin/*.dat/*.model`銆乣env/`銆乣png/jpg/svg`锛夛紝瀵煎嚭 `resolveEnabledProfiles` / `collectEnabledProfilePatterns` / `buildIgnoreSnapshot` / `validateCustomExclusionPatterns`
  - 鏂板 manifest 浠撳偍 `CheckpointManifestRepository.ts`锛歚checkpoints/cp_xxx/manifest.json` 鍘熷瓙鍐欏叆锛坱mp+rename锛夈€佹寜 ID 鍔犺浇锛堝唴瀛樼紦瀛橈級銆佹棫璁板綍杩佺Щ锛堜粠 `CheckpointRecord.fileHashes/fileStats/emptyDirs/changes` 鐢熸垚骞惰惤鐩橈紝骞傜瓑锛夈€佹崯鍧忓洖閫€銆佸啓鍏ュけ璐ユ竻鐞?tmp锛涙柊鏍煎紡璁板綍鏃犳枃浠跺搱甯屾椂杩佺Щ浜х墿涓虹┖鍒欒繑鍥?null 骞舵樉寮忔姤銆屽瓨妗ｆ暟鎹己澶便€嶈€岄潪鍋囨垚鍔?  - 鏂板 `CheckpointQueryService.ts`锛坓etCheckpoints / getAllConversationsWithCheckpoints / getDirectorySize / pruneMissingBackupCheckpointRecords / 瀛ゅ効鐩綍娓呯悊锛変笌 `CheckpointRetentionService.ts`锛坈leanupOldCheckpoints / mergeCheckpointIntoSuccessor锛夛紝`CheckpointManager` 浠?2000+ 琛屾媶鍒嗕负鍗忚皟灞?+ 涓夋湇鍔★紙CPF-12锛?  - 鏂板鍏变韩鏈夌晫骞跺彂姹?`checkpointConcurrency.ts`锛坮unBounded锛岄閿欏仠姝㈠彇鏂颁换鍔″苟姝ｇ‘浼犳挱锛夛紝鍒涘缓澶嶅埗銆佹仮澶?鍒犻櫎寰幆銆佺洰褰曞ぇ灏忕粺璁″潎鏀逛负鏈夌晫骞跺彂锛堥粯璁?8锛夛紙CPF-06锛?  - 浼氳瘽鍏冩暟鎹簿绠€锛氬畬鏁?`fileHashes` / `fileStats` 浠庝細璇濆厓鏁版嵁杩佸埌鐙珛 manifest锛屽厓鏁版嵁鍙繚鐣?`CheckpointSummary`锛坒ileCount / backupBytes / excludedCount / manifestVersion 绛夛級锛沗getCheckpoints` / `getAllConversationsWithCheckpoints` 鍙笅鍙戞憳瑕侊紝鏃у瓨妗ｇ己瀛楁鏃舵寜闇€鎳掓壂鎻忓苟鍐欏洖缂撳瓨锛涚鐩樺崰鐢ㄥ垱寤烘椂鐩存帴璁板綍锛坄backupBytes`锛夛紝璁剧疆椤典笉鍐嶉噸澶嶉€掑綊鎵弿锛圕PF-01/02/09/10锛?  - 娴佸紡璺緞涓嬪彂鎽樿鍖栵細宸ュ叿鎵ц鏈熼棿鐨?checkpoints chunk 缁?`CheckpointService.toStreamSummary()` 鍓ョ `fileHashes/fileStats`锛宍loadConversationForView` 涓庢祦寮忎袱鏉¤矾寰勯兘涓嶅啀鍚?webview 浼犲叏閲忓搱甯屾槧灏勶紙CPF-03/04 琛ュ叏锛?  - 瀛樻。鎿嶄綔鎺ュ叆杩涘害涓庡彇娑堬細Manager 缁存姢杩涜涓搷浣滅姸鎬侊紙闃舵 / 宸插鐞?/ 鎬绘暟锛孉bortController锛夛紝create / restore / deleteAll / deleteBatch 鍏ㄦ帴鍏ワ紱RestoreEngine 鏀寔 `signal` + `onProgress`锛涙柊澧?`checkpoint.getOperationProgress` / `checkpoint.cancelOperation` handler 涓庡墠绔疆璇?+ 鍙栨秷鎸夐挳锛圕PF-11锛?  - 鍙宸ュ叿鎵规涓嶅啀鍒涘缓瀛樻。锛圕PF-05锛夛細`tool_batch` 鍒ゅ畾鏀逛负鍩轰簬鐪熷疄宸ュ叿鍚嶉泦鍚?`toolNames.some(name => configuredTools.includes(name))`锛宍read_file + search_in_files(search)` 绛夌函鍙鎵规涓嶅缓瀛樻。锛宍search_in_files` 浠?replace 妯″紡璁″叆
  - 鍘嗗彶杩藉姞鏀逛负 append-only 灏炬鍐欏叆锛圚IS-01/02锛夛細鏅€氳拷鍔犲彧鍐欐渶鍚庝竴娈碉紙涓嶈冻 200 鏉★級鎴栨柊寤轰笅涓€娈?+ 鏇存柊 index锛屼笉鍐嶅叏閲忛噸鍐欐墍鏈夋锛涘啓鍏ラ『搴忋€屼复鏃跺熬娈?鈫?鍘熷瓙鏇挎崲 鈫?涓存椂 index 鈫?鍘熷瓙鏇挎崲銆嶏紝index 鏄彁浜ょ偣锛涘垹闄?/ 缂栬緫 / 鍥炴。 / 鍒嗘敮鍒囨崲浠嶈蛋鍏ㄩ噺閲嶅啓锛沗TranscriptRepository` 鏂板 `appendContents` 鐩撮€?append锛宍addContent` / `addBatch` 绾拷鍔犺矾寰勪笉鍐嶈鍏ㄩ噺锛坒unctionResponse 淇濈暀 mutate 鍘婚噸瀹夊叏缃戯級
  - 鏂板娈电骇 LRU 缂撳瓨 `history/HistorySegmentCache.ts`锛圚IS-06锛夛細閿?`conversationId + segmentFile + revision(+mtime)`锛岄粯璁?32 娈碉紝鍐欏悗 / 鍒犻櫎浼氳瘽澶辨晥锛涘娈佃鍙栨敼鏈夌晫骞跺彂锛堝苟鍙?4锛夛紙HIS-05锛?  - 鍚屼竴宸ュ叿杩唬鍐呭鐢ㄥ巻鍙插揩鐓э紙HIS-03/04锛夛細`getHistoryForAPIFrom(contents)` / `getStatsFrom(contents)` 鏀寔鍩轰簬宸插姞杞藉巻鍙茬洿鎺ユ牸寮忓寲锛宍ContextTrimService` 涓嶅啀姣忔閲嶆柊璇诲叏閲忓巻鍙?  - 鍏冩暟鎹啓鍏ュ悎骞讹紙HIS-09锛夛細鏂板 `conversation.updateSummary({conversationId, messageCount, preview})` 涓€娆¤鍐欏畬鎴愶紝鍓嶇娴佸紡缁撴潫鍚庣敱 3 娆?`setCustomMetadata` 鏀逛负 1 娆★紱`updatedAt` 鐢卞巻鍙叉彁浜ょ粺涓€缁存姢
  - 瀵硅瘽鍒楄〃鎵归噺鍏冩暟鎹紙HIS-10锛夛細鏂板 `conversation.getConversationMetadataBatch`锛?6 骞跺彂銆佹瘡椤?鈮?00锛夛紝鍓嶇 `loadMoreConversations` 涓€娆?IPC 鎷変竴椤垫憳瑕侊紝娑堥櫎閫愬璇?IPC
  - 鐢ㄩ噺绱㈠紩澧為噺缁存姢锛圚IS-08锛夛細鏂板 `UsageIndexStore.appendUsage` / `appendUsageMessages`锛屾櫘閫氳拷鍔犲姪鎵嬫秷鎭彧杩藉姞鏉＄洰锛屽垹闄?/ 缂栬緫 / 鍥炴。 / 绱㈠紩鎹熷潖鎵嶅叏閲忛噸寤猴紱`getMetadata` 瀹屾暣鎬ф鏌ュ彧璇?`history.index.json` 缁撴瀯锛屼笉鍐嶈В鏋愭湯娈靛巻鍙诧紙HIS-11锛?  - 瀛恆gent 鐢ㄩ噺褰掗泦锛堢敤鎴烽渶姹傦級锛氬瓙agent 姣忚疆 generate 鐨?`usageMetadata` 浠?`source: 'subagent'` 鏉＄洰杩藉姞鍒板彂璧峰畠鐨勪富浼氳瘽鐢ㄩ噺绱㈠紩锛坄appendUsageIndexMessages`锛夛紝鐢ㄩ噺缁熻 totals / byConversation / byModel / byDay 鑷姩鍖呭惈瀛恆gent 娑堣€楋紝`ConversationUsage` 鏂板 `subagentTokens` 缁嗗垎锛涚储寮?stale 鍏ㄩ噺閲嶅缓鏃朵繚鐣欏凡鏈?subagent 鏉＄洰锛涙棤涓讳細璇濆綊灞炴椂璺宠繃褰掗泦
  - 鍓嶇娓叉煋浼樺寲锛圚IS-12/13锛夛細鍙娑堟伅绐楀彛澧為噺缂撳瓨锛堝悓寮曠敤 + 闀垮害 + 棣栧熬鎸囩汗 O(1) 鏍￠獙锛屾祦寮忓熬鏇挎崲 O(1)锛岀粨鏋勫彉鏇村洖閫€鍏ㄩ噺杩囨护锛夛紱瀵硅瘽鍔犺浇鍏堟覆鏌撴渶鍚庝竴椤靛啀寮傛琛ユ媺鏇存棭鍘嗗彶锛坄backfillInitialVisibleWindow`锛屼笌鐢ㄦ埛涓婃媺 / 鍙戦€?/ 娴佸紡骞跺彂鏃舵斁寮冨悎骞讹級
  - 鍏冩暟鎹啓鍏ュ師瀛愬寲锛歚saveMetadata` 鏀逛负涓存椂鏂囦欢 + rename 鍘熷瓙鏇挎崲锛屼换浣曞啓璺緞涓嶅啀鍙兘鐣欎笅鎴柇鐨?meta.json
  - 瀛恆gent 璁剧疆鏂板銆岄粯璁よ凯浠ｆ鏁般€嶏紙鐢ㄦ埛闇€姹傦級锛歚SubAgentsConfig.defaultMaxIterations`锛堥粯璁?80锛屼笂闄?1000锛夛紝executor 鍙栧€?`per-agent maxIterations > 鍏ㄥ眬榛樿 > 50`锛岃缃〉鍏ㄥ眬閰嶇疆鍖烘柊澧炶緭鍏ユ锛?~1000锛夛紝`subagents.list` 杩斿洖璇ュ€?
## [1.3.1-1] - 2026-08-02

### Fixed
  - 淇瀛樺偍璺緞杩佺Щ鐩綍濂楀▋锛堟棤闄愰€掑綊锛夛細`copyDirectory` 澶嶅埗鏃惰嫢鐩爣浣嶄簬婧愮洰褰曞唴閮ㄤ細鏃犻檺閫掑綊鑷垜澶嶅埗锛坢kdir 鐩爣鍚?readdir 婧愪細鐪嬪埌鍒氬垱寤虹殑瀛愮洰褰曪紝涓€娆¤閫夊瓙璺緞鍙骇鐢熸暟鍗冨眰宓屽锛夛紝鐜版娴嬪埌鐩爣鍦ㄦ簮鍐呴儴鏃惰烦杩囷紱璺緞閲嶅彔鍒ゆ柇鏀圭敤 `realpath`锛宩unction/绗﹀彿閾炬帴鏃犳硶缁曡繃闃插濞?  - 淇瀛樺偍璺緞杩佺Щ鏁版嵁瀹夊叏锛氭簮/鐩爣浜掔浉鍖呭惈鏃跺厛澶嶅埗鍒颁复鏃剁洰褰曚腑杞紝閬垮厤娓呯悊鏃х洰褰曟椂鍒犻櫎鏂版暟鎹紱澶嶅埗澶辫触缁堟杩佺Щ骞跺洖婊氾紝婧愭暟鎹笌閰嶇疆淇濇寔涓嶅彉锛涘け璐ユ椂鎭㈠鍘?`customDataPath`/鐘舵€侊紝涓嶅啀閿欒鍒囧洖榛樿璺緞锛涜矾寰勯獙璇佹敼鐢ㄩ殢鏈轰复鏃剁洰褰曪紝涓嶅啀瑕嗙洊鐢ㄦ埛 `.limcode-test` 鏂囦欢
  - 淇鍓嶇璇 `config.customPath`锛堝悗绔瓧娈典负 `customDataPath`锛夊鑷撮噸缃寜閽案杩滅鐢紱搴旂敤/閲嶇疆鎸夐挳澧炲姞鏄庣‘鎻愮ず锛堢┖璺緞銆佸凡鏄粯璁よ矾寰勬椂涓嶅啀闈欓粯鏃犲弽搴旓級
  - 淇璁剧疆椤垫弿杩板崟琛屾埅鏂細宸ュ叿鍚嶄笌鎻忚堪鏀圭敤 i18n锛?7 涓伐鍏峰叏瑕嗙洊锛夛紝鎻忚堪鏍峰紡鏀硅嚜鍔ㄦ崲琛岋紙pre-wrap锛夛紝涓嶅啀鎴柇

### Changed
  - 宸ュ叿璋冪敤鏍煎紡閫夐」绉婚櫎銆岋紙鎺ㄨ崘锛夈€嶆爣璁帮細JSON 杈圭晫鏍囪涓嶅啀鏍囨敞涓烘帹鑽愰」锛堝師鐢?Function Calling 宸叉槸涓绘祦閫夋嫨锛夛紝娓犻亾璁剧疆涓嬫媺涓庢彁绀烘枃妗堢殑涓?鑻?鏃ヤ笁璇悓姝ヨ皟鏁?  - 璁剧疆椤?i18n 琛ュ叏锛氬伐鍏疯缃?鑷姩鎵ц/瀛樻。鐐?瀛愪唬鐞?妯″瀷绠＄悊鐨勫垎绫诲垎缁勫叏閲忎笁璇炕璇戯紙璁板繂/瀹℃煡/杩涘害/鎶€鑳?璁捐/閫氱煡/浠ｇ悊绛?16 绫伙級锛屾秷闄ら噸澶嶃€屽叾浠栥€嶅垎缁勶紱Prompt 璁剧疆琛?`modules.MEMORY`锛堣蹇嗙郴缁燂級涓夎锛涜蹇嗚缃?Raw Memory Entries 绛夌‖缂栫爜鑻辨枃鏀硅蛋 i18n
  - 瀛樺偍璺緞璁剧疆 UI 鏀圭増锛氬悎骞躲€屽綋鍓嶅瓨鍌ㄨ矾寰?鑷畾涔夎矾寰勩€嶄负鍗曚竴杈撳叆妗嗭紝鏂板 馃搧 鏂囦欢澶归€夋嫨鎸夐挳锛坄storagePath.selectFolder`锛夛紝鍒犻櫎鍐椾綑鐨勮縼绉绘暟鎹寜閽紱`validatePath` 鏀惧鈥斺€斿厑璁搁€夋嫨榛樿璺緞/瀛愯矾寰?鏅€氶潪绌虹洰褰曪紙濡傛闈級锛屼粎鎷掔粷宸插寘鍚墿灞曟暟鎹瓙鐩綍锛坈onversations/checkpoints 绛夛級鐨勭洰鏍囷紝闃叉鏁版嵁娣峰悎
  - 娓犻亾璁剧疆鏂板缓閰嶇疆锛氭敼涓烘ā鎬侀伄缃╁脊绐楋紙鐐瑰嚮閬僵鍙栨秷锛夛紝閰嶇疆鍚嶇О涓虹┖鏃剁孩妗?鎻愮ず
  - 鏇存柊鎵╁睍涓庢椿鍔ㄦ爮鍥炬爣锛氬競鍦哄浘鏍囩敱 1024px 瑙掕壊鎻掔敾锛?.4MB锛夋浛鎹负 256px 涓夎壊鍓奖鐗堬紙73KB锛屾繁钃濆渾瑙掓柟搴曪紝灏忓昂瀵歌鲸璇嗗害鏇村ソ锛夛紱娲诲姩鏍?渚ц竟鏍忓浘鏍囩敱 VS Code 榛樿鍦扮悆鏇挎崲涓鸿鑹插壀褰憋紝骞惰繘涓€姝ユ敼涓虹函杞粨绾跨锛坒ill=none + stroke=currentColor 闅忎富棰樼潃鑹诧紝Douglas-Peucker 璺緞绠€鍖?716鈫?61 鐐癸紝24px 涓嬫棤閿娇锛?
### Added
  - 鏂板 `StoragePathManager` 鍗曞厓娴嬭瘯锛? 椤癸級锛氳鐩栬縼绉讳腑杞€佸け璐ュ洖婊氥€乺ealpath 閲嶅彔鍒ゆ柇銆佽矾寰勯獙璇佷复鏃剁洰褰曠瓑瀹夊叏鍔犲浐琛屼负
