...
- 🔀 **多实例（便携式）**：所有数据写入应用目录下 `data/`（不写 AppData/Program Files），复制应用目录即得独立实例，也可 `--user-data-dir` 显式指定
...
应用为**便携式设计**：所有数据（会话、设置、工作区、记忆、用量、缓存）默认写入
**应用自身目录下的 `data/`**，不写入系统路径（AppData / Program Files）。

- 复制一份应用目录（zip 版解压目录 / win-unpacked / 源码项目文件夹），即可得到
  一个完全独立的实例——两个实例的数据互不影响，可同时运行。
- 也可为单个实例显式指定数据目录：
...
> 注意：默认便携数据目录随应用目录移动；卸载/删除应用目录即删除全部数据。
